...
public abstract class AbstractExperimentRunner implements ExperimentRunner {
     ...
     *
     * @param modelProto a model prototype class from which we create a new
     * instance
     * @param config the configuration for this model
     * @param producers the data producers to be attached to the model
     * @return the model ready to submit
     */
    protected Model setupModel(Model modelProto, Configuration config, Experiment experiment) {
    ...
    protected void setupDataConsumers(Experiment experiment) {
        Collection<? extends DataConsumer> consumers = experiment.getConsumers();
        for (DataConsumer consumer : consumers) {

            DataProducer producer = experiment.getProducer(consumer.getProducerID());
